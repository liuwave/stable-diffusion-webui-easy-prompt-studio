class EPSElementCreator {
  ...
  static openButton({ onClick }) {
    const button = EPSElementCreator.baseButton("🔯Easy Prompt标签选择", {
      size: "sm",
      color: "secondary",
    });
  ...
  static tagButton({ title, onClick, onRightClick, color = "primary" }) {
    ...
    button.title = "左键添加，右键删除；ctrl+鼠标：反向提示";
...
class EasyPromptStudio {
  ...
  render() {
    ...
    if (this.tags && Object.keys(this.tags).length) {
      dropDown.value = Object.keys(this.tags)[0];
      this.dropDownChangeEvent(container)(dropDown.value);
    }

    return container;
  }

  dropDownChangeEvent(container) {
    const that = this;
    return (selected) => {
      const content = (container || gradioApp()).querySelector(
        `#${that.CONTENT_ID}`
      );
  ...
  renderDropdown() {
    const dropDown = EPSElementCreator.dropDown(
      ...
      {
        onChange: this.dropDownChangeEvent(),
      }
    );
  ...
  renderTab(tags, prefix) {
    ...
    Object.keys(tags).forEach((key, index) => {
      ...
      button.addEventListener("click", (e) => {
        ...
        const targetIndex = e.target.getAttribute("data-target") || 0;

        const contentList = tabs.querySelectorAll(".content-item");
        for (let i = 0; i < contentList.length; i++) {
          contentList[i].classList.remove("show");
        }
        contentList[targetIndex].classList.add("show");
      });
...
onUiLoaded(async () => {
  ...
  const button = EPSElementCreator.openButton({
    onClick: () => {
      ...
      easyPromptStudio.changeVisibility(
        tagArea,
        (easyPromptStudio.visible = !easyPromptStudio.visible)
      );
    },
  });

  const reloadButton = gradioApp().getElementById(
    "easy_prompt_studio_reload_button"
  );
  reloadButton.addEventListener("click", async () => {
    await easyPromptStudio.init();
  });
  ...
  container.classList.add("easy_prompt_studio_container");
